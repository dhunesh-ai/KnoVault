...
import { dbQueue, localInsert, localUpdate, localDelete } from './db';
...
export const storageManager = {
  ...
  getNotes: async (params?: { category?: string; search?: string; pinned_only?: boolean }): Promise<any[]> => {
    ...
    try {
      ...
      // Update SQLite Cache cleanly via dbQueue
  ...
  createNote: async (data: any): Promise<any> => {
    ...
    try {
      ...
      // Update SQLite Cache asynchronously via dbQueue
      await dbQueue.write(async (db) => {
        await db.runAsync(
          ...
          note.id, note.title, note.content, note.category, note.is_secure ? 1 : 0, note.is_pinned ? 1 : 0, note.is_completed ? 1 : 0, note.note_type, note.is_favorite ? 1 : 0, note.checklist_items ? JSON.stringify(note.checklist_items) : null, note.field_notes ? JSON.stringify(note.field_notes) : null, note.user_id, note.created_at, note.updated_at
        );
      });
      ...
      logger.error('Failed to create note on Neon, queuing locally:', e);
  ...
  deleteNote: async (id: number): Promise<void> => {
    ...
    if (storageMode === 'gdrive') {
      await localDelete('Notes', id);
      try {
        const mapping: any = await dbQueue.read(async (db) => {
          return db.getFirstAsync('SELECT drive_file_id FROM GoogleDriveSync WHERE entity = ? AND record_id = ?', ['Notes', id]);
        });
  ...
  syncGoogleDrive: async (): Promise<boolean> => {
    ...
    try {
      ...
      // Sync Notes
      const notes = await dbQueue.read(async (db) => db.getAllAsync('SELECT * FROM Notes WHERE is_deleted = 0'));
      for (const note of notes as any[]) {
        const fileId = await googleDrive.uploadJsonFile(token, 'Notes', `note_${note.remote_id || note.id}.json`, sqlToAppNote(note));
        await dbQueue.write(async (db) => db.runAsync('INSERT OR REPLACE INTO GoogleDriveSync (entity, record_id, drive_file_id) VALUES (?, ?, ?)', 'Notes', note.id, fileId));
      }

      // Sync Goals
      const goals = await dbQueue.read(async (db) => db.getAllAsync('SELECT * FROM Goals WHERE is_deleted = 0'));
      for (const goal of goals as any[]) {
        ...
        await dbQueue.write(async (db) => db.runAsync('INSERT OR REPLACE INTO GoogleDriveSync (entity, record_id, drive_file_id) VALUES (?, ?, ?)', 'Goals', goal.id, fileId));
      }

      // Sync Reminders
      const reminders = await dbQueue.read(async (db) => db.getAllAsync('SELECT * FROM Reminders WHERE is_deleted = 0'));
      for (const rem of reminders as any[]) {
        ...
        await dbQueue.write(async (db) => db.runAsync('INSERT OR REPLACE INTO GoogleDriveSync (entity, record_id, drive_file_id) VALUES (?, ?, ?)', 'Reminders', rem.id, fileId));
      }

      // Sync ImportantDays
      const days = await dbQueue.read(async (db) => db.getAllAsync('SELECT * FROM ImportantDays WHERE is_deleted = 0'));
      for (const day of days as any[]) {
        ...
        await dbQueue.write(async (db) => db.runAsync('INSERT OR REPLACE INTO GoogleDriveSync (entity, record_id, drive_file_id) VALUES (?, ?, ?)', 'ImportantDays', day.id, fileId));
      }
