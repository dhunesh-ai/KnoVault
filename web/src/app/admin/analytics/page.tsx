...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight">Platform Growth & Usage Analytics</h1>
        <p className="text-xs text-slate-400 mt-1">Time series tracking for Daily Active Users, Registrations, and AI Requests.</p>
      </div>
                ...
                <tbody className="divide-y divide-slate-800/60 font-mono">
                  {charts.map((c: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-800/30">
                      <td className="py-3 px-4 text-slate-200 font-bold">{c.date}</td>
                      <td className="py-3 px-4 text-emerald-400">{c.dau}</td>
                      <td className="py-3 px-4 text-indigo-400">+{c.new_users}</td>
                      <td className="py-3 px-4 text-purple-400">{c.ai_requests}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
