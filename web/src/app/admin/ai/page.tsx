  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">AI Intelligence & Token Monitoring</h1>
          <p className="text-xs text-slate-400 mt-1">
            Monitor Groq AI model performance, latency, token consumption, and feature distribution.
          </p>
        </div>
        <button onClick={fetchStats} className="px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-200 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl">
          <span className="text-xs text-slate-400 font-medium">Total AI Conversations</span>
          <p className="text-2xl font-bold text-slate-100 mt-1">{aiStats?.total_conversations || 0}</p>
        </div>
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl">
          <span className="text-xs text-slate-400 font-medium">Estimated Tokens Today</span>
          <p className="text-2xl font-bold text-indigo-400 mt-1">{aiStats?.total_tokens_today || 0}</p>
        </div>
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl">
          <span className="text-xs text-slate-400 font-medium">Avg Response Time</span>
          <p className="text-2xl font-bold text-emerald-400 mt-1">{aiStats?.avg_response_time_ms} ms</p>
        </div>
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl">
          <span className="text-xs text-slate-400 font-medium">Active Model</span>
          <p className="text-lg font-bold text-purple-400 mt-1 font-mono">{aiStats?.current_model}</p>
        </div>
      </div>

      {/* Feature Usage Breakdown */}
      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl space-y-4">
        <h3 className="text-sm font-bold text-slate-200">AI Feature Breakdown</h3>
          ...
          {Object.entries(aiStats?.features_breakdown || {}).map(([key, val]: any) => (
            <div key={key} className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold text-slate-300 capitalize">
                ...
                <span className="font-mono text-indigo-400">{val} Requests</span>
              </div>
