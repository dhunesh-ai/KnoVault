...
export default function SecurityPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Security Center & Intrusion Audit</h1>
        <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Monitor failed logins, suspicious access attempts, and authentication anomalies.</p>
      </div>
              ...
              {loading ? (
                ...
                logs.map((l: any) => (
                  <tr key={l.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 px-4 text-slate-600 dark:text-slate-400">{new Date(l.created_at).toLocaleString()}</td>
                    ...
                    <td className="py-3 px-4 text-slate-900 dark:text-slate-200 font-bold">{l.user_email || `ID #${l.user_id}` || 'Anonymous'}</td>
                    <td className="py-3 px-4 text-indigo-600 dark:text-indigo-400 font-semibold">{l.ip_address || '127.0.0.1'}</td>
                    <td className="py-3 px-4 capitalize text-slate-600 dark:text-slate-400">{l.platform || 'web'}</td>
                    <td className="py-3 px-4 text-slate-600 dark:text-slate-400 truncate max-w-xs">{l.details || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
