...
export default function BackupPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight">Database Backup & Disaster Recovery</h1>
        <p className="text-xs text-slate-400 mt-1">Export encrypted database metadata archives and system configuration snapshots.</p>
      </div>

      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
          <Database size={18} className="text-indigo-400" />
          <h2 className="text-sm font-bold text-slate-200">System Database Export</h2>
        </div>
        ...
        {backupRes && (
          <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs space-y-2">
            ...
            <p className="font-mono text-[11px] text-slate-300">File: {backupRes.backup_file}</p>
            <p className="font-mono text-[10px] text-slate-400">Generated: {backupRes.timestamp}</p>
          </div>
        )}
      </div>
    </div>
