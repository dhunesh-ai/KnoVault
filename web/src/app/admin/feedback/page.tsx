...
export default function FeedbackPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold tracking-tight">User Feedback & Support Tickets</h1>
        <p className="text-xs text-slate-400 mt-1">Review user bug reports, app suggestions, and support requests.</p>
      </div>

      {/* Tabs */}
      <div className="flex items-center gap-2 p-1 bg-slate-900 border border-slate-800 rounded-xl w-fit text-xs font-semibold">
        <button
          onClick={() => setTab('bugs')}
          className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-all ${
            tab === 'bugs' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          <Bug size={15} />
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-all ${
            tab === 'features' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          <Lightbulb size={15} />
        ...
        {loading ? (
          ...
          feedback.bug_reports.length === 0 ? (
            ...
            feedback.bug_reports.map((b: any) => (
              <div key={b.id} className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
                ...
                <div className="text-[11px] text-slate-400 font-mono bg-slate-950 p-2.5 rounded-xl border border-slate-800">
          ...
          feedback.feature_suggestions.length === 0 ? (
            ...
            feedback.feature_suggestions.map((f: any) => (
              <div key={f.id} className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-200 text-xs">{f.title}</span>
                    <span className="px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 text-[10px] uppercase font-mono">
                      Priority: {f.priority}
                    </span>
                  </div>
                ...
                <p className="text-[11px] text-emerald-400 font-medium">Expected Benefit: {f.expected_benefit}</p>
              </div>
            ))
          )
        )}
